...
{
    public class GetEventDraftById
    {
            ...
            public async Task<Result<EventItem>> Handle(Contract request, CancellationToken cancellationToken)
            {
                try
                {
                    if (request.UserRole == "Secretary" || request.UserRole == "Recruiter")
                        return Result.Fail<EventItem>("Acesso Negado");

                    if (String.IsNullOrEmpty(request.Id))
                        return Result.Fail<EventItem>("Id do Evento não Informado");

                    var evtId = ObjectId.Parse(request.Id);
                    var query = await _db.Database
                        .GetCollection<EventItem>("EventsDrafts")
                ...
                {
                    return Result.Fail<EventItem>(ex.Message);
                }
            }
