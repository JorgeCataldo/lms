...
{
    public class GetTracksPagedQuery
    {
        ...
        public class Handler : IRequestHandler<Contract, Result<PagedTrackItems>>
        {
            ...
            {                
                ...
                var qry = await collection.FindAsync(filters,
                    options: request.PageSize > 0 ? options : null,
            ...
            private FilterDefinition<TrackItem> SetFilters(Contract request, List<ObjectId> specialTrackIds)
            {
                ...
                if (request.Filters.Attending.HasValue)
                {
                    if (request.Filters.Attending.Value)
                    {
                        filters = filters & (
                            builder.Eq(x => x.Recommended, true) |
                    ...
                    {
                        filters = filters & (
                            builder.Eq(x => x.Recommended, false)
                        );
                    }
                }
                ...
                if (request.UserRole == "BusinessManager")
                {
                    if (request.Filters.Published.HasValue)
                    {
                        filters = filters & builder.In(x => x.Id, specialTrackIds);
                    }
                }
