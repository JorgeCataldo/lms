...
{
    public class UpdateActivationStatusCommand
    {
        ...
        public class Handler : IRequestHandler<Contract, Result<bool>>
        {
            ...
            public async Task<Result<bool>> Handle(Contract request, CancellationToken cancellationToken)
            {
                ...
                if (act == null)
                {
                    var newAct = Activation.Create(request.Active, request.Type, request.Title, request.Text, request.Percentage);

                    if (newAct.IsFailure)
                        return Result.Fail<bool>("Ocorreu um erro na ativação");
                ...
                else
                {
                    ...
                    await _db.ActivationsCollection.ReplaceOneAsync(t =>
                        t.Type == request.Type, act,
                        cancellationToken: cancellationToken
                    );
                }

                return Result.Ok(request.Active);
            }
        }
    }
}
