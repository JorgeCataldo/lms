...
{
    public class SendEventReactionEvaluationEmailCommand
    {
                    ...
                    if (eventSchedule != null)
                    {
                        if (!request.UsersId.IsNullOrEmpty())
                        {
                            if (request.UsersId.Count == 1)
                            {
                                ...
                                {
                                    await SendEmailToUser(ObjectId.Parse(request.UsersId[0]), eventDb, request.EventScheduleId, cancellationToken);
                                    return Result.Ok(request);
                                }
                                return Result.Fail<Contract>("So é possivel enviar email individual quando o geral for enviado");
                            }
                            else
                            {
                                if (!eventSchedule.SentReactionEvaluationEmails)
                                {
                                    ...
                                    eventSchedule.SentReactionEvaluationEmails = true;
                                    await _db.EventCollection.ReplaceOneAsync(t => t.Id == eventDb.Id, eventDb, cancellationToken: cancellationToken);
                                    return Result.Ok(request);
                                }
                                return Result.Fail<Contract>("Emails ja enviados");
                            }
                        }
                        return Result.Fail<Contract>("Usuario(s) inválido(s)");
                    }
