...
namespace Api.Controllers
{
    ...
    public class UserController: BaseController
    {
        ...
        [HttpGet("getUserByCarrerTerm")]
        public async Task<IActionResult> getUserByCarrerTerm(GetUserCareerByTermQuery.Contract request)
        {
            var result = await this.Send(request);
            if (result.IsFailure)
                return RestResult.CreateHttpResponse(result);

            var bytes = result.Data.ToExcel("usuarios");
            return File(new MemoryStream(bytes), "application/octet-stream", "usuarios.xlsx");
        }
    }
}